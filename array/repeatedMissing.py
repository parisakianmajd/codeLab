...
def repeatedNumber2(A):
    repeated = 0
    missing = 0
    # XOR A with integers 1..N
    # all elements of A will cancel out except for the repeated and missing element
    
    xorA = reduce(lambda x, y: x^y, A + range(1, len(A)+1))
    
    # All the bits that are set in xor1 will be set in either repeated or missing num
    # Take any bit of xor and divide the elements of array into two set one with that bit set and the other with the unset bit

    setBit = xorA & 2**1  # get the bit at index 1
    for a in xrange(len(A)):
        if A[a] & setBit:
            missing ^= A[a]
        else:
            repeated ^= A[a]
    for a in xrange(1,len(A)+1):
        if a & setBit:
            missing ^= a
        else:
            repeated ^= a
...
# Solution 2. Using elements as index and mark the visited places
...
# Solution 3. Find the total sum n*(n+1)/2 and subtract all elements from it to find the missing one
# because of overflow problem this solution is not used.
# x: repeated, y: missing
# sum S = n * (n+1)/2 - x
# product P = 1 * 2 * 3 * ... * n * y / x
